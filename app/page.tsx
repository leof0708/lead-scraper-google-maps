...
function UsageBar({ used }: { used: number }) {
  ...
  const color =
    used >= FREE_LIMIT ? "bg-red-500" : used >= WARN_AT ? "bg-amber-500" : "bg-red-800";
  return (
...
export default function Home() {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    if (needed > remaining) {
      setError(`This search needs ~${needed} API calls but you only have ${remaining} left this month.`);
      return;
    }
    setLoading(true);
    setError("");
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="flex items-start justify-between mb-10 gap-6 flex-wrap">
          <div>
            ...
            <p className="text-[#7a5050] text-sm ml-[52px]">
          ...
          <div className="flex gap-3">
            ...
            <div className={`bg-[#130707] border rounded-xl px-5 py-3 text-right min-w-[170px] ${
              ...
              <div className={`text-2xl font-bold tabular-nums ${
                limitReached ? "text-red-400" : nearLimit ? "text-amber-400" : "text-white"
              }`}>
                ...
                <label className="block text-xs font-semibold text-[#7a5050] uppercase tracking-wider mb-2">
                  ...
                  {maxLeads && Number(maxLeads) > 0 && (
                    <span className={`ml-2 normal-case font-normal ${
                      ...
                      {Math.ceil(Number(maxLeads) / 20) > remaining && " (exceeds quota)"}
                    </span>
                  )}
                </label>
                <input
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {searched && (
                <div className="ml-auto flex items-center gap-2 text-xs">
                  {apiCallsMade > 0 && (
                    <span className="text-[#5a3535]">
                      {apiCallsMade} call{apiCallsMade !== 1 ? "s" : ""} used
                    </span>
                  )}
                  {dupLeads.length > 0 && (
        ...
        {loading && (
          <div className="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-[#130707] border border-[#2d1212] rounded-xl p-5 animate-pulse">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="h-4 bg-[#2d1212] rounded w-2/5 mb-3" />
                    <div className="h-3 bg-[#1e0808] rounded w-1/4 mb-4" />
                    <div className="h-3 bg-[#1e0808] rounded w-3/5 mb-2" />
                    <div className="h-3 bg-[#1e0808] rounded w-2/5" />
                  </div>
                  <div className="h-8 w-28 bg-[#1e0808] rounded-lg ml-4" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2.5">
            {places.map((place, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`border rounded-xl p-5 transition-all duration-200 group ${
                    alreadySaved
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <Badge variant={alreadySaved ? "dup" : "new"}>
                          {alreadySaved ? "In DB" : "New"}
                        </Badge>
                      ...
                      {place.rating && (
                        <div className="mb-3">
                          ...
                          {place.reviewCount && (
                            <span className="text-[#5a3a3a] text-xs ml-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1.5 text-sm">
                        {place.address && (
                          <div className="flex gap-2">
                            <span className="text-[#4a2020] shrink-0 mt-0.5">
                        ...
                        {place.phone && (
                          <div className="flex gap-2">
                            ...
                            <a href={`tel:${place.phone}`} className="text-red-400/80 hover:text-red-300 transition-colors">
                              {place.phone}
                            </a>
                          </div>
                        )}
